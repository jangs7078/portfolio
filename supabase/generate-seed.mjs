#!/usr/bin/env node
// Generate seed.sql for Supabase SQL Editor
// Run: node supabase/generate-seed.mjs > supabase/seed.sql

// ─── Asset Metadata ───
// All Sheet 4 values are in USD. KRW assets: value_native = value_usd * fx_rate

const HOLDINGS = [
  // Samsung Securities (KR brokerage)
...
const INVESTMENTS = [
  ...
  { key: 'inv_gold', asset_type: 'commodity', name: 'Gold', currency: 'USD', ticker: 'GCW00', price_source: 'comex' },
...
// Fund LP breakdown (KRW)
const FUND_LP = { total: 80_000_000, breakdown: { inv_krew: 30_000_000, inv_infobank_fund: 50_000_000 } };

// Angel breakdown (KRW) - 100 shares each assumed
const ANGEL = {
  total: 180_000_000,
  breakdown: {
    inv_mobilix: 20_000_000,
    inv_mrmind: 40_000_000,
    inv_taeju: 70_000_000,
    inv_quad: 27_000_000,
    inv_starnex: 3_000_000,
    inv_gravity: 20_000_000,
  }
};

// Investment current values and quantities (as of ~4/1/2026)
const INV_CURRENT = {
  inv_scalev: { current_value: 390276, cost_basis: null, quantity: 97569 },
  inv_scale: { current_value: 293250, cost_basis: null, quantity: 48875 },
  inv_gold: { current_value: 1092574, cost_basis: null, quantity: 227, unit_label: 'oz' },
  inv_silver: { current_value: 7608, cost_basis: null, quantity: 100, unit_label: 'oz' },
  inv_krew: { current_value: 30_000_000, cost_basis: 30_000_000, quantity: null },
  inv_infobank_fund: { current_value: 50_000_000, cost_basis: 50_000_000, quantity: null },
  inv_mobilix: { current_value: 20_000_000, cost_basis: 20_000_000, quantity: 100 },
  inv_mrmind: { current_value: 40_000_000, cost_basis: 40_000_000, quantity: 100 },
  inv_taeju: { current_value: 70_000_000, cost_basis: 70_000_000, quantity: 100 },
  inv_quad: { current_value: 27_000_000, cost_basis: 27_000_000, quantity: 100 },
  inv_starnex: { current_value: 3_000_000, cost_basis: 3_000_000, quantity: 100 },
  inv_gravity: { current_value: 20_000_000, cost_basis: 20_000_000, quantity: 100 },
};

// Current holdings shares (as of 4/22/2026 from Sheet 8)
const CURRENT_SHARES = {
  ...
  h_schwab: 1, h_krw_hana: 473156634,
};

// ─── Sheet 4: Per-date values (all in USD) ───
// Columns: date, fx, 039290, 246720, 000660, CEG, SGOV, TLT, KRW_SAM, SIL, SHLD, XLE, SOXX, BOTZ, GEV, GOOGL, META, MSFT, PANW, CLRB, USD, SCHWAB, SCALEV, SCALE, GCW00, KRW_HANA, SIW00, FUND, ANGEL
const SNAPSHOT_DATA = [
...
// Column indices for SNAPSHOT_DATA
const C = {
  DATE: 0, FX: 1,
  '039290': 2, '246720': 3, '000660': 4, CEG: 5, SGOV: 6, TLT: 7, KRW_SAM: 8,
  SIL: 9, SHLD: 10, XLE: 11, SOXX: 12, BOTZ: 13, GEV: 14, GOOGL: 15, META: 16, MSFT: 17, PANW: 18, CLRB: 19, USD: 20,
  SCHWAB: 21, SCALEV: 22, SCALE: 23, GCW00: 24, KRW_HANA: 25, SIW00: 26, FUND: 27, ANGEL: 28,
};

// Map holding key → column index in SNAPSHOT_DATA
const HOLDING_COL = {
  h_039290: C['039290'], h_246720: C['246720'], h_000660: C['000660'],
  h_ceg: C.CEG, h_sgov: C.SGOV, h_tlt: C.TLT, h_krw_sam: C.KRW_SAM,
  h_sil: C.SIL, h_shld: C.SHLD, h_xle: C.XLE, h_soxx: C.SOXX,
  h_botz: C.BOTZ, h_gev: C.GEV, h_googl: C.GOOGL, h_meta: C.META,
  h_msft: C.MSFT, h_panw: C.PANW, h_clrb: C.CLRB, h_usd: C.USD,
  h_schwab: C.SCHWAB, h_krw_hana: C.KRW_HANA,
};

// Map investment key → column index
const INV_COL = {
  inv_scalev: C.SCALEV, inv_scale: C.SCALE, inv_gold: C.GCW00, inv_silver: C.SIW00,
};

// ─── Sheet 8: Share counts per date ───
// Format: { holdingKey: sharesCount, ... }
const SHARES_BY_DATE = {
  '2026-03-01': {
    h_039290:45963, h_246720:4588, h_000660:0, h_ceg:100, h_sgov:3282, h_tlt:4451,
    h_krw_sam:1116346838,
    h_sil:0, h_shld:873, h_xle:1150, h_soxx:0, h_botz:0, h_gev:12, h_googl:123, h_meta:43, h_msft:25, h_panw:0, h_clrb:1602,
    h_usd:1229915, h_schwab:1, h_krw_hana:472057725,
    inv_scalev:89500, inv_scale:48875, inv_gold:227, inv_silver:100,
  },
  '2026-03-02': {
    h_039290:45963, h_246720:4588, h_000660:0, h_ceg:100, h_sgov:3282, h_tlt:4451,
    h_krw_sam:1116346838,
    h_sil:172, h_shld:873, h_xle:1150, h_soxx:0, h_botz:263, h_gev:12, h_googl:123, h_meta:43, h_msft:0, h_panw:0, h_clrb:1602,
    h_usd:1209757, h_schwab:1, h_krw_hana:472057725,
    inv_scalev:89500, inv_scale:48875, inv_gold:227, inv_silver:100,
  },
  '2026-03-04': {
    h_039290:45963, h_246720:4588, h_000660:0, h_ceg:100, h_sgov:3282, h_tlt:4451,
    h_krw_sam:1116346838,
    h_sil:536, h_shld:873, h_xle:1150, h_soxx:0, h_botz:263, h_gev:12, h_googl:123, h_meta:43, h_msft:0, h_panw:0, h_clrb:1602,
    h_usd:1169892, h_schwab:1, h_krw_hana:472057725,
    inv_scalev:89500, inv_scale:48875, inv_gold:227, inv_silver:100,
  },
  '2026-03-09': {
    h_039290:45963, h_246720:4588, h_000660:0, h_ceg:100, h_sgov:7175, h_tlt:0,
    h_krw_sam:1115628575,
    h_sil:536, h_shld:873, h_xle:2211, h_soxx:0, h_botz:263, h_gev:12, h_googl:123, h_meta:43, h_msft:0, h_panw:0, h_clrb:1602,
    h_usd:1109958, h_schwab:1, h_krw_hana:472057725,
    inv_scalev:89500, inv_scale:48875, inv_gold:227, inv_silver:100,
  },
  '2026-03-10': {
    h_039290:45963, h_246720:4588, h_000660:0, h_ceg:100, h_sgov:7175, h_tlt:0,
    ...
    h_sil:536, h_shld:873, h_xle:2211, h_soxx:0, h_botz:263, h_gev:12, h_googl:123, h_meta:43, h_msft:0, h_panw:0, h_clrb:1602,
    h_usd:1109958, h_schwab:1, h_krw_hana:472057725,
    inv_scalev:90463, inv_scale:48875, inv_gold:227, inv_silver:100,
  },
  '2026-03-11': {
    h_039290:45963, h_246720:4588, h_000660:0, h_ceg:100, h_sgov:7175, h_tlt:0,
    h_krw_sam:1115731816,
    h_sil:1132, h_shld:1664, h_xle:2211, h_soxx:0, h_botz:263, h_gev:12, h_googl:123, h_meta:43, h_msft:0, h_panw:0, h_clrb:1602,
    h_usd:989852, h_schwab:1, h_krw_hana:472057725,
    inv_scalev:90463, inv_scale:48875, inv_gold:227, inv_silver:100,
  },
  '2026-03-18': {
    h_039290:45963, h_246720:4588, h_000660:0, h_ceg:100, h_sgov:7175, h_tlt:0,
    h_krw_sam:1115731816,
    h_sil:1809, h_shld:1664, h_xle:2211, h_soxx:0, h_botz:563, h_gev:24, h_googl:188, h_meta:76, h_msft:0, h_panw:0, h_clrb:1602,
    h_usd:868663, h_schwab:1, h_krw_hana:472057725,
    inv_scalev:90463, inv_scale:48875, inv_gold:227, inv_silver:100,
  },
  '2026-03-26': {
    h_039290:45963, h_246720:4588, h_000660:0, h_ceg:100, h_sgov:7175, h_tlt:0,
    h_krw_sam:1115731816,
    h_sil:1809, h_shld:1664, h_xle:2211, h_soxx:0, h_botz:563, h_gev:35, h_googl:188, h_meta:76, h_msft:0, h_panw:0, h_clrb:1602,
    h_usd:858798, h_schwab:1, h_krw_hana:472057725,
    inv_scalev:90463, inv_scale:48875, inv_gold:227, inv_silver:100,
  },
  '2026-03-30': {
    h_039290:45963, h_246720:4588, h_000660:0, h_ceg:100, h_sgov:7175, h_tlt:0,
    h_krw_sam:1116700668,
    h_sil:1809, h_shld:1664, h_xle:3498, h_soxx:0, h_botz:563, h_gev:35, h_googl:188, h_meta:76, h_msft:0, h_panw:0, h_clrb:1602,
    h_usd:778745, h_schwab:1, h_krw_hana:472057725,
    inv_scalev:90463, inv_scale:48875, inv_gold:227, inv_silver:100,
  },
  '2026-04-01': {
    h_039290:45963, h_246720:4588, h_000660:0, h_ceg:100, h_sgov:7175, h_tlt:0,
    h_krw_sam:1116700668,
    h_sil:1809, h_shld:1664, h_xle:1798, h_soxx:0, h_botz:563, h_gev:35, h_googl:188, h_meta:76, h_msft:0, h_panw:0, h_clrb:1602,
    h_usd:727020, h_schwab:1, h_krw_hana:472057725,
    inv_scalev:97569, inv_scale:48875, inv_gold:227, inv_silver:100,
  },
};

// Resolve shares for any date by finding the latest SHARES_BY_DATE entry <= date
const SHARE_DATES = Object.keys(SHARES_BY_DATE).sort();
function getShares(date) {
  let best = SHARE_DATES[0];
  for (const d of SHARE_DATES) {
...
w(`-- Portfolio Seed Data`);
w(`-- Generated from Google Sheets data (3/1/2026 - 4/1/2026)`);
w(`-- Run in Supabase SQL Editor (bypasses RLS)`);
w(``);
w(`DO $$`);
w(`DECLARE`);
w(`  uid UUID;`);
...
// Account variables
w(`  acc_samsung UUID;`);
w(`  acc_robinhood UUID;`);
w(`  acc_schwab UUID;`);
w(`  acc_hana UUID;`);

// Holding variables
for (const h of HOLDINGS) w(`  ${h.key} UUID;`);

// Investment variables
for (const inv of INVESTMENTS) w(`  ${inv.key} UUID;`);

w(`BEGIN`);
w(``);
w(`  -- Get the first user (must exist)`);
w(`  SELECT id INTO uid FROM auth.users LIMIT 1;`);
w(`  IF uid IS NULL THEN RAISE EXCEPTION 'No user found in auth.users. Sign up first.'; END IF;`);
w(``);

// ─── Accounts ───
w(`  -- Accounts`);
const accounts = [
  { var: 'acc_samsung', name: '삼성증권', type: 'brokerage', country: 'KR' },
  { var: 'acc_robinhood', name: 'Robinhood', type: 'brokerage', country: 'US' },
  { var: 'acc_schwab', name: 'Charles Schwab', type: 'brokerage', country: 'US' },
  { var: 'acc_hana', name: '하나은행', type: 'savings', country: 'KR' },
];
for (const a of accounts) {
  w(`  INSERT INTO accounts (user_id, name, type, country) VALUES (uid, ${esc(a.name)}, ${esc(a.type)}, ${esc(a.country)}) RETURNING id INTO ${a.var};`);
}
w(``);
...
for (const h of HOLDINGS) {
  ...
  w(`  INSERT INTO holdings (user_id, account_id, ticker, name, asset_type, shares, currency)`);
  w(`    VALUES (uid, ${accMap[h.account]}, ${esc(h.ticker)}, ${esc(h.name)}, ${esc(h.asset_type)}, ${num(shares)}, ${esc(h.currency)})`);
  w(`    RETURNING id INTO ${h.key};`);
}
w(``);

// ─── Private Investments ───
w(`  -- Private Investments`);
for (const inv of INVESTMENTS) {
  const cur = INV_CURRENT[inv.key];
  w(`  INSERT INTO private_investments (user_id, asset_type, name, current_value, cost_basis_total, currency, quantity, unit_label, ticker, price_source)`);
  w(`    VALUES (uid, ${esc(inv.asset_type)}, ${esc(inv.name)}, ${num(cur.current_value)}, ${num(cur.cost_basis)}, ${esc(inv.currency)}, ${num(cur.quantity)}, ${esc(cur.unit_label || null)}, ${esc(inv.ticker)}, ${esc(inv.price_source)})`);
  w(`    RETURNING id INTO ${inv.key};`);
}
w(``);
...
for (const row of SNAPSHOT_DATA) {
  const date = row[C.DATE];
  const fx = row[C.FX];
  const shares = getShares(date);

  w(`  -- ${date}`);
  w(`  INSERT INTO snapshots (user_id, date, holding_id, investment_id, ticker, value_native, currency, value_usd, fx_rate, price_per_unit) VALUES`);
  ...
  for (const h of HOLDINGS) {
    ...
    if (shareCount > 0 && valueNative > 0) {
      if (h.asset_type === 'cash') {
        ...
        ppu = String(Math.round((valueNative / shareCount) * 100) / 100);
      }
    }

    rows.push(`    (uid, ${esc(date)}, ${h.key}, NULL, ${esc(h.ticker)}, ${num(valueNative)}, ${esc(h.currency)}, ${num(valueUsd)}, ${num(fx)}, ${ppu})`);
  }

  // Investment snapshots: ScaleV, Scale, Gold, Silver
  for (const [invKey, colIdx] of Object.entries(INV_COL)) {
    const inv = INVESTMENTS.find(i => i.key === invKey);
    const valueUsd = row[colIdx];
    ...
    let ppu = 'NULL';
    if (qty > 0 && valueNative > 0) {
      ppu = String(Math.round((valueNative / qty) * 100) / 100);
    }

    const ticker = inv.ticker || inv.name;
    rows.push(`    (uid, ${esc(date)}, NULL, ${invKey}, ${esc(ticker)}, ${num(valueNative)}, ${esc(inv.currency)}, ${num(valueUsd)}, ${num(fx)}, ${ppu})`);
  }

  // Fund LP breakdown
  const fundTotalUsd = row[C.FUND];
  for (const [invKey, krwValue] of Object.entries(FUND_LP.breakdown)) {
    const proportion = krwValue / FUND_LP.total;
    const valueUsd = Math.round(fundTotalUsd * proportion * 100) / 100;
    const valueNative = krwValue; // constant KRW
    const inv = INVESTMENTS.find(i => i.key === invKey);
    const ticker = inv.ticker || inv.name;
    rows.push(`    (uid, ${esc(date)}, NULL, ${invKey}, ${esc(ticker)}, ${num(valueNative)}, 'KRW', ${num(valueUsd)}, ${num(fx)}, NULL)`);
  }

  // Angel breakdown
  const angelTotalUsd = row[C.ANGEL];
  for (const [invKey, krwValue] of Object.entries(ANGEL.breakdown)) {
    const proportion = krwValue / ANGEL.total;
    const valueUsd = Math.round(angelTotalUsd * proportion * 100) / 100;
    const valueNative = krwValue; // constant KRW, 100 shares
    const inv = INVESTMENTS.find(i => i.key === invKey);
    const ticker = inv.ticker || inv.name;
    const ppu = krwValue / 100; // 100 shares assumed
    rows.push(`    (uid, ${esc(date)}, NULL, ${invKey}, ${esc(ticker)}, ${num(valueNative)}, 'KRW', ${num(valueUsd)}, ${num(fx)}, ${num(ppu)})`);
  }
...
w(`  RAISE NOTICE 'Seed complete: % snapshot dates, % accounts, % holdings, % investments',`);
w(`    ${SNAPSHOT_DATA.length}, ${accounts.length}, ${HOLDINGS.length}, ${INVESTMENTS.length};`);
w(``);
w(`END $$;`);
